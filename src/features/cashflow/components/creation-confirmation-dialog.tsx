...
import { CashflowInput } from "../schemas";
import { ActionResponse } from "@/types";
import { UseMutateAsyncFunction } from "@tanstack/react-query";

interface CreatationConfirmationDialogProps {
  form: UseFormReturn<CashflowInput>;
  isPending: boolean;
  isFormValid: boolean;
  mutationFn: UseMutateAsyncFunction<ActionResponse, ActionResponse, CashflowInput, unknown>;
}
...
export function CreatationConfirmationDialog({ form, mutationFn, isPending, isFormValid }: CreatationConfirmationDialogProps) {
  ...
  return (
    <AlertDialog open={open} onOpenChange={setOpen}>
      <AlertDialogTrigger render={<Button disabled={isFormValid}>Tambah Mutasi Kas</Button>} />
